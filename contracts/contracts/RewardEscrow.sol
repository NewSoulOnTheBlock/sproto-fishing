...
contract RewardEscrow is PausableLite {
    ...
    constructor(IERC20Lite asset_, address rewardSigner_) { require(address(asset_)!=address(0),'asset=0'); require(rewardSigner_!=address(0),'signer=0'); asset=asset_; rewardSigner=rewardSigner_; DOMAIN_SEPARATOR=keccak256(abi.encode(keccak256('EIP712Domain(string name,string version,uint256 chainId,address verifyingContract)'), keccak256(bytes('SprotoFishingRewards')), keccak256(bytes('1')), block.chainid, address(this))); }
