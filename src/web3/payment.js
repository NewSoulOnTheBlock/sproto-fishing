// Robinhood Chain ERC-20 payment helpers. Keeps legacy $SPROTO function names so
...
export function isOnChainPayEnabled() { return Boolean(SPROTO_MINT && TIDE_TREASURY && currentWalletAddress()); }

export function paymentConfig() {
  return { mint: SPROTO_MINT || null, model: "erc20_transfer_to_treasury", treasury: TIDE_TREASURY || null, decimals: DEFAULT_DECIMALS, symbol: TIDE_SYMBOL, enabled: isOnChainPayEnabled() };
}
...
export async function payTide(uiAmount, { memo } = {}) {
  ...
  const balance = await fetchErc20Balance(payer, SPROTO_MINT);
  ...
  return sendTransaction({ to: SPROTO_MINT, data, value: "0x0" });
}
