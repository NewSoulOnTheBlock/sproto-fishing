...
import { decodeAbiParameters as decodeAbiParameters2, encodeFunctionData as encodeFunctionData4, toHex as toHex5 } from "viem";
...
import { hexToNumber as hexToNumber2, isAddressEqual, numberToHex as numberToHex6 } from "viem";
...
function ecdsa(Point, hash, ecdsaOpts = {}) {
  ...
  function validateSigLength(bytes, format) {
    ...
    const sizer = format === "compact" ? size4 : format === "recovered" ? size4 + 1 : void 0;
...
import { keccak256 as keccak2562, numberToHex, slice as slice3, toHex as toHex3 } from "viem";
...
function assertGetCapabilitiesParams(params) {
  ...
  if (typeof params[0] !== "string" || !isAddress2(params[0])) {
    throw standardErrors.rpc.invalidParams();
  }
  if (params.length === 2) {
...
function compute16ByteHash(input) {
  return slice3(keccak2562(toHex3(input)), 0, 16);
}
...
import { hexToBytes as hexToBytes2, numberToHex as numberToHex2, stringToBytes, trim as trim3 } from "viem";
...
function convertCredentialToJSON({ webauthn, signature, id }) {
  ...
  return {
    ...
    response: {
      authenticatorData: arrayBufferToBase64Url(hexToBytes2(webauthn.authenticatorData)),
...
function asn1EncodeSignature(r, s) {
  const rBytes = hexToBytes2(trim3(numberToHex2(r)));
  const sBytes = hexToBytes2(trim3(numberToHex2(s)));
...
import { hexToString, isHex as isHex2, numberToHex as numberToHex3 } from "viem";

// node_modules/@base-org/account/dist/sign/base-account/utils/createSmartAccount.js
import { BaseError as BaseError2, decodeFunctionData, encodeAbiParameters, encodeFunctionData, encodePacked, hashMessage as hashMessage2, hashTypedData as hashTypedData2, parseSignature, size as size3, stringToHex } from "viem";
...
async function sign3({ hash, owner }) {
  ...
  throw new BaseError2("`owner` does not support raw sign.");
}
...
function wrapSignature(parameters) {
  ...
  const signatureData = (() => {
    if (size3(parameters.signature) !== 65)
...
import { decodeFunctionData as decodeFunctionData2, isAddress as isAddress3, pad as pad3 } from "viem";
...
async function findOwnerIndex({ address, client, publicKey, factory, factoryData }) {
  ...
  if (!code && factory && factoryData) {
    const initData = decodeFunctionData2({
      abi: factoryAbi,
      data: factoryData
    });
...
function formatPublicKey(publicKey) {
  if (isAddress3(publicKey)) {
    return pad3(publicKey);
  }
  return publicKey;
}

// node_modules/@base-org/account/dist/sign/base-account/utils/handleAddSubAccountOwner.js
import { decodeAbiParameters, encodeFunctionData as encodeFunctionData2, numberToHex as numberToHex4, toHex as toHex4 } from "viem";
import { waitForCallsStatus as waitForCallsStatus2 } from "viem/experimental";
...
async function handleAddSubAccountOwner({ ownerAccount, globalAccountRequest }) {
  ...
  const callsResult = await waitForCallsStatus2(client, {
    id: callsId
  });
...
import { encodeFunctionData as encodeFunctionData3, erc20Abi, hexToBigInt as hexToBigInt2, numberToHex as numberToHex5 } from "viem";
async function handleInsufficientBalanceError({ errorData, globalAccountAddress, subAccountAddress, client, request, subAccountRequest, globalAccountRequest }) {
  ...
  if (userChoice === "update_permission") {
    if (spendPermissionRequests.length === 1) {
      ...
      const message = createSpendPermissionMessage({
        spendPermission: {
          token: spendPermission.token,
          allowance: numberToHex5(spendPermission.requiredAmount * BigInt(defaultMultiplier)),
          period: defaultPeriod,
          account: globalAccountAddress,
          spender: subAccountAddress,
          start: 0,
          end: 281474976710655,
          salt: numberToHex5(BigInt(Math.floor(Math.random() * Number.MAX_SAFE_INTEGER))),
          extraData: "0x"
        },
        chainId
      });
  ...
  const transferCalls = spendPermissionRequests.map((spendPermission) => {
    ...
    if (isNative) {
      return {
        ...
        value: numberToHex5(spendPermission.requiredAmount),
        data: "0x"
      };
    }
    return {
  ...
  const subAccountCallData = encodeFunctionData3({
    abi,
    functionName: "executeBatch",
    args: [
      originalSendCallsParams.calls.map((call) => {
        ...
        return {
          target: call.to,
          value: hexToBigInt2((_a2 = call.value) !== null && _a2 !== void 0 ? _a2 : "0x0"),
...
var Signer = class {
  ...
  async _request(request) {
    ...
    switch (request.method) {
      ...
      case "eth_accounts": {
        ...
        (_e = this.callback) === null || _e === void 0 ? void 0 : _e.call(this, "connect", { chainId: numberToHex6(this.chain.id) });
      ...
      case "eth_chainId":
        return numberToHex6(this.chain.id);
      ...
      case "wallet_connect": {
        ...
        (_j = this.callback) === null || _j === void 0 ? void 0 : _j.call(this, "connect", { chainId: numberToHex6(this.chain.id) });
        return result;
      }
      // Sub Account Support
      ...
      case "coinbase_fetchPermissions": {
        ...
        const requestedChainId = hexToNumber2((_k = completeRequest.params) === null || _k === void 0 ? void 0 : _k[0].chainId);
  ...
  async handleGetCapabilitiesRequest(request) {
    ...
    const filterChainNumbers = new Set(filterChainIds.map((chainId) => hexToNumber2(chainId)));
    const filteredCapabilities = Object.fromEntries(Object.entries(capabilities).filter(([capabilityKey]) => {
      try {
        const capabilityChainNumber = hexToNumber2(capabilityKey);
  ...
  const sdk = {
    ...
    subAccount: {
      ...
      addOwner: async ({ address, publicKey, chainId }) => {
        ...
        const calls = [];
        if (publicKey) {
          const [x, y2] = decodeAbiParameters2([{ type: "bytes32" }, { type: "bytes32" }], publicKey);
          calls.push({
            to: subAccount.address,
...
import { encodeFunctionData as encodeFunctionData5, parseUnits } from "viem";
