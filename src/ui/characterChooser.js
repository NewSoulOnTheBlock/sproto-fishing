    ...
    <div class="cc-root">
      ...
      <div class="cc-grid" role="listbox" aria-label="Choose your character">
        ${CHARACTERS.map((c) => {
          ...
          return `
          <button type="button" class="cc-chip${locked ? " is-locked" : ""}" role="option" data-id="${c.id}" data-locked="${locked ? "1" : ""}" aria-selected="false"${locked ? ` title="Unlock in Shop → Anglers (${formatMoney(c.price)} $BITCOIN)"` : ""}>
  ...
  chips.forEach((chip) => {
    chip.addEventListener("click", () => {
      if (chip.dataset.locked) {
        ...
        events.emit("toast", { msg: `Unlock ${c.name} in Shop → Anglers (${formatMoney(c.price)} $BITCOIN)`, kind: "warn" });
        return;
      }
