...
export class LeaderboardUI {
  ...
  tweetUrl(text) {
    const params = new URLSearchParams({ text, url: 'https://sprotofishing.fun' });
  ...
  medal(entry) {
    ...
    const text = `${icon} ${who} is ${place} on the Sproto Fishing leaderboard with ${formatMoney(Number(entry.total_earned))} earned! 🎣 Can you out-fish them?`;
  ...
  speciesMedal(c, i, speciesName) {
    ...
    const text = `${icon} ${who} holds the ${place}-biggest ${speciesName} on Sproto Fishing — ${Number(c.size_cm).toFixed(1)}cm! 🎣 Think you can beat it?`;
    return `<div class="trophy" data-share="${this.esc(this.tweetUrl(text))}" title="Share on X" role="button">${icon}</div>`;
  }
