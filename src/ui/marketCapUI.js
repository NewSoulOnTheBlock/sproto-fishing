// Title-screen dashboard meta: live $BITCOIN market cap pill + the contract-address
...
export class MarketCapUI {
  ...
  setupFooter() {
    ...
    if (!SPROTO_MINT_ADDRESS) return;

    if (addrEl) {
      addrEl.textContent = shortAddr(SPROTO_MINT_ADDRESS, 6, 6);
      addrEl.title = SPROTO_MINT_ADDRESS;
    }
  ...
  async copyAddress() {
    try {
      if (navigator.clipboard?.writeText) {
        await navigator.clipboard.writeText(SPROTO_MINT_ADDRESS);
      } else {
        const ta = document.createElement("textarea");
        ta.value = SPROTO_MINT_ADDRESS;
