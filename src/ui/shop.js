...
// Now with DUAL PAYMENT OPTIONS: pay with $BITCOIN or ETH!
...
import { formatSol, tideToSol } from "../web3/solPayment.js";
...
export class ShopUI {
  ...
  renderGear(catKey) {
    ...
    // Option-A pricing: each item is anchored to an ETH price and its $BITCOIN cost is
    // the live ETH-equivalent (same model as bait), so paying in $BITCOIN always
      ...
      if (equipped) {
        ...
        if (walletConnected && tidePayAvailable) {
          ...
          offChainBtn.innerHTML = `
            <span class="pay-label">Pay with $BITCOIN</span>
          ...
          btn.textContent = afford ? `Buy ${formatMoney(tideCost)}` : "Not enough $BITCOIN";
          btn.disabled = !afford;
          if (afford) {
  ...
  renderBait() {
    ...
    intro.innerHTML =
      `Wagering mode: <b>buy bait with $BITCOIN</b>, spend <b>1 bait per cast</b>, then win by selling the fish you catch. Tiers 1–3 are fast server/provably-fair packs; tiers 4–6 are premium Gamba-ready casts.`;
    ...
    BAITS.forEach((b) => {
      ...
      usdgBtn.innerHTML = `
        <span class="pay-label">Buy ×${qty} bait · $BITCOIN onchain</span>
      ...
      if (walletConnected) {
        ...
        usdgBtn.title = "Connect wallet to buy bait with $BITCOIN";
      }
  ...
  renderSell() {
    ...
    S.inventory.forEach((fish, idx) => {
      ...
      sbfBtn.textContent = "Sell $BITCOIN";
    ...
    sellAllBtn.textContent = "Sell All $BITCOIN";
   ...
   * @param {string} method - 'tide-offchain' or 'tide-onchain'
   */
  ...
  async buyAngler(id, method, solAmount = 0, tideCost = 0) {
    ...
    if (method === "tide-offchain") {
      ...
      try {
        events.emit("toast", { msg: "Processing $BITCOIN transfer...", kind: "info" });
        const sig = await payTide(tideCost, { memo: `sproto:angler:${id}` });
        economy.grantAnglerOnChain(id, sig);
        economy.selectAngler(id);
        audio.play("buy");
        events.emit("toast", {
          msg: `${c.name} unlocked with $BITCOIN · ${shortAddress(sig, 6, 6)}`,
          kind: "gold",
          href: explorerTxUrl(sig),
        });
        events.emit("wallet:refresh");
      } catch (e) {
        console.error("[sproto] on-chain $BITCOIN angler payment failed", e);
        audio.play("error");
   ...
   * @param {string} method - Payment method: 'tide-offchain' or 'tide-onchain'
   */
  async buyGear(catKey, idx, method, solAmount = 0, tideCost = 0) {
    const item = GEAR[catKey][idx];
    
    if (method === 'tide-offchain') {
      // Standard off-chain $BITCOIN purchase
      const res = economy.buyGear(catKey, idx, tideCost);
      if (res.ok) {
        audio.play("buy");
        events.emit("toast", { msg: `${item.name} purchased with $BITCOIN`, kind: "success" });
      } else {
        audio.play("error");
        events.emit("toast", { msg: res.reason, kind: "warn" });
      }
      ...
      // On-chain $BITCOIN payment (treasury transfer)
      try {
        events.emit("toast", { msg: "Processing $BITCOIN transfer...", kind: "info" });
        ...
        audio.play("buy");
        events.emit("toast", {
          msg: `${item.name} unlocked with $BITCOIN · ${shortAddress(sig, 6, 6)}`,
          kind: "gold",
          href: explorerTxUrl(sig),
        });
        events.emit("wallet:refresh");
      } catch (e) {
        console.error("[sproto] on-chain $BITCOIN payment failed", e);
        audio.play("error");
        events.emit("toast", { msg: e?.message ?? "On-chain payment failed", kind: "warn" });
      } finally {
        this.render();
