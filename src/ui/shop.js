...
// Now with DUAL PAYMENT OPTIONS: pay with $SPROTO or ETH!
...
export class ShopUI {
  ...
  renderGear(catKey) {
    ...
    // Option-A pricing: each item is anchored to an ETH price and its $SPROTO cost is
    // the live ETH-equivalent (same model as bait), so paying in $SPROTO always
    ...
    GEAR[catKey].forEach((item, idx) => {
      ...
      if (equipped) {
        ...
        if (walletConnected && (solPayAvailable || tidePayAvailable)) {
          ...
          // Off-chain $SPROTO button (always available)
          ...
          offChainBtn.innerHTML = `
            <span class="pay-label">Pay with $SPROTO</span>
          ...
          if (afford) {
            ...
            offChainBtn.title = "Not enough $SPROTO";
          }
          ...
          btn.textContent = afford ? `Buy ${formatMoney(tideCost)}` : "Not enough $SPROTO";
          btn.disabled = !afford;
          if (afford) {
  ...
  renderBait() {
    ...
    BAITS.forEach((b) => {
      ...
      usdgBtn.innerHTML = `
        <span class="pay-label">Buy ×${qty} bait · $SPROTO onchain</span>
      ...
      if (walletConnected) {
        ...
        usdgBtn.title = "Connect wallet to buy bait with $SPROTO";
      }
  ...
  renderSell() {
    ...
    S.inventory.forEach((fish, idx) => {
      ...
      sbfBtn.textContent = "Sell $SPROTO";
    ...
    sellAllBtn.textContent = "Sell All $SPROTO";
  ...
  renderAnglers() {
    ...
    // Option-A pricing: $SPROTO cost is the live ETH-equivalent of each angler's
  ...
  async buyBaitWith(id, qty, method, solAmount = 0, tideCost = 0) {
    ...
    if (method === "tide-offchain") {
      ...
      try {
        ...
        console.error("[sproto] ETH bait payment failed", e);
        audio.play("error");
      ...
      try {
        events.emit("toast", { msg: "Approve $SPROTO if needed, then confirm BaitStore purchase...", kind: "info" });
        ...
        console.error("[sproto] verified BaitStore purchase failed", e);
        audio.play("error");
  ...
  async buyAngler(id, method, solAmount = 0, tideCost = 0) {
    ...
    if (method === "tide-offchain") {
      ...
      try {
        events.emit("toast", { msg: "Processing ETH payment...", kind: "info" });
        const sig = await paySol(solAmount, { memo: `sproto:angler:${id}` });
        ...
        console.error("[sproto] ETH angler payment failed", e);
        audio.play("error");
        events.emit("toast", { msg: e?.message ?? "ETH payment failed", kind: "warn" });
      } finally {
        this.render();
      ...
      try {
        events.emit("toast", { msg: "Processing $SPROTO transfer...", kind: "info" });
        const sig = await payTide(tideCost, { memo: `sproto:angler:${id}` });
        economy.grantAnglerOnChain(id, sig);
        economy.selectAngler(id);
        audio.play("buy");
        events.emit("toast", {
          msg: `${c.name} unlocked with $SPROTO · ${shortAddress(sig, 6, 6)}`,
          kind: "gold",
          href: explorerTxUrl(sig),
        });
        events.emit("wallet:refresh");
      } catch (e) {
        console.error("[sproto] on-chain $SPROTO angler payment failed", e);
        audio.play("error");
  ...
  async buyGear(catKey, idx, method, solAmount = 0, tideCost = 0) {
    ...
    if (method === 'tide-offchain') {
      // Standard off-chain $SPROTO purchase
      const res = economy.buyGear(catKey, idx, tideCost);
      if (res.ok) {
        audio.play("buy");
        events.emit("toast", { msg: `${item.name} purchased with $SPROTO`, kind: "success" });
      } else {
        audio.play("error");
        events.emit("toast", { msg: res.reason, kind: "warn" });
      }
      ...
      try {
        events.emit("toast", { msg: "Processing ETH payment...", kind: "info" });
        const sig = await paySol(solAmount, { memo: `sproto:gear:${catKey}:${idx}` });
        ...
        console.error("[sproto] ETH payment failed", e);
        audio.play("error");
        events.emit("toast", { msg: e?.message ?? "ETH payment failed", kind: "warn" });
      } finally {
        this.render();
      ...
      // On-chain $SPROTO payment (treasury transfer)
      try {
        events.emit("toast", { msg: "Processing $SPROTO transfer...", kind: "info" });
        const sig = await payTide(tideCost, { memo: `sproto:gear:${catKey}:${idx}` });
        
        economy.grantGearOnChain(catKey, idx, sig);
        audio.play("buy");
        events.emit("toast", {
          msg: `${item.name} unlocked with $SPROTO · ${shortAddress(sig, 6, 6)}`,
          kind: "gold",
          href: explorerTxUrl(sig),
        });
        events.emit("wallet:refresh");
      } catch (e) {
        console.error("[sproto] on-chain $SPROTO payment failed", e);
        audio.play("error");
        events.emit("toast", { msg: e?.message ?? "On-chain payment failed", kind: "warn" });
      } finally {
        this.render();
