// Wallet HUD panel — connect/disconnect, address pill, ETH + $SPROTO balances.
//
...
const MIN_HOLD_REQUIREMENT = 1_000_000; // 1,000,000 $SPROTO
...
export class WalletPanel {
  constructor() {
    ...
    this.account = null;
    this.lastTideBalanceUi = 0; // on-chain $SPROTO (UI units) for the hold gate
    ...
    // Re-render when the player's earned $SPROTO changes so the Withdraw row
  ...
  render() {
    if (this.account) {
      ...
      if (mintConfigured && earned > 0) {
        ...
        withdrawHtml += `<button class="btn btn-withdraw btn-withdraw-compact" disabled title="Withdrawals activate once $SPROTO goes live">Withdraw soon™</button>`;
      }
      if (solSaleValue > 0) {
