// Wallet HUD panel — connect/disconnect, address pill, ETH + $BITCOIN balances.
//
...
import { shortAddress, explorerAddressUrl, explorerTxUrl, NETWORK, SPROTO_MINT, TIDE_SYMBOL, NATIVE_SYMBOL } from "../web3/chain.js";
...
const MIN_HOLD_REQUIREMENT = 1_000_000; // 1,000,000 $BITCOIN
...
export class WalletPanel {
  constructor() {
    ...
    this.account = null;
    this.lastTideBalanceUi = 0; // on-chain $BITCOIN (UI units) for the hold gate
    ...
    // Re-render when the player's earned $BITCOIN changes so the Withdraw row
  ...
  render() {
    if (this.account) {
      ...
      const mintConfigured = !!SPROTO_MINT;
      ...
      if (mintConfigured && earned > 0) {
        ...
        withdrawHtml += `<button class="btn btn-withdraw btn-withdraw-compact" disabled title="Withdrawals activate once $BITCOIN goes live">Withdraw soon™</button>`;
      }
      if (solSaleValue > 0) {
