...
// The Render service URL is deployment-specific (Render appends a random
// suffix), so there is no safe default — a stale one silently proxies to a
// host that does not exist. Set RENDER_API_BASE in the Vercel project env.
const DEFAULT_RENDER_API_BASE = "";
...
function forwardHeaders(req) {
  ...
  headers["x-forwarded-host"] = req.headers.host || "www.sprotofishing.fun";
...
export default function handler(req, res) {
  ...
  if (!base) {
    return res.status(503).json({
      ...
      detail: "Set RENDER_API_BASE to the SPROTO FISHING Render API URL.",
    });
  }
