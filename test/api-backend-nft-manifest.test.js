...
test('Vercel backend proxy serves Fish NFT manifest locally without Render secret', async () => {
  const req = { method: 'GET', query: { path: '/api/nft/manifest' }, headers: { origin: 'https://www.sprotofishing.fun' } };
  let statusCode = 200;
  const headers = {};
  let body = null;
  const res = {
  ...
  assert.equal(headers['access-control-allow-origin'], 'https://www.sprotofishing.fun');
