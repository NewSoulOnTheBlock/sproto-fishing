...
**Play it now:** https://sproto-theta-tawny.vercel.app

Forked from [bridge-mind/tideline](https://github.com/bridge-mind/tideline)
(Three.js procedural fishing sim, MIT). Sproto adds:
...
node smoke.mjs https://sproto-theta-tawny.vercel.app/   # smokes the deploy
```
...
| `VITE_SPROTO_MINT` | $SBF SPL token mint address. Until set, the wallet panel shows `—` for $SBF balance and the burn buttons stay hidden. When set, gear & map UIs render a `🔥 Burn N $SBF` button beside the regular Buy/Unlock button. |
| `VITE_SPROTO_DECIMALS` | Decimals of the $SBF mint (default 9, matching SOL). |
...
When a wallet connects, the active save key switches from `sproto_save_v1` to
`sproto_save_v1:<wallet_address>`. The anonymous local save is migrated into a
new wallet's slot on first connection so signing in never feels like a wipe.

### On-chain $SBF payments (deflationary burn)

When `VITE_SPROTO_MINT` is set and a wallet is connected, gear and location
...
3. Attaches a memo identifying the purchase (`sproto:gear:rods:2`, `sproto:loc:river`).
...
- `🔥 Burn N $SBF` for gear & locations (dormant until `VITE_SPROTO_MINT` set).
