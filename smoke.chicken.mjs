...
const result = await page.evaluate(async () => {
  const out = {};
  const t = window.SPROTO;
  if (!t) return { ok: false, why: 'no SPROTO' };
  ...
  // Easier: walk the bundle by intercepting via SPROTO handle
  ...
  // Reach into the catch card via SPROTO — main.js doesn't expose it,
